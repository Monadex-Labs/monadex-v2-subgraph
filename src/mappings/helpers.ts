...
export const FACTORY_ADDRESS = '0xd829c1d3649dbc3fd96d3d22500ef33a46daae46'
...
export function fetchTokenName(tokenAddress: Address): string {
   // Check if token is already indexed
   let token = Token.load(tokenAddress.toHexString())
   if (token && token.name && token.name != 'unknown') {
     return token.name
   }
...
export function fetchTokenTotalSupply(tokenAddress: Address): BigInt {
   // Check if token is already indexed and has total supply
   let token = Token.load(tokenAddress.toHexString())
   if (token && token.totalSupply && !token.totalSupply.equals(ZERO_BI)) {
     return token.totalSupply as BigInt
   }

  let contract = ERC20.bind(tokenAddress)
  let totalSupplyValue: BigInt = BigInt.zero()
...
export function fetchTokenDecimals(tokenAddress: Address): BigInt {
    ...
    if (token && token.decimals) {
      return token.decimals as BigInt
    }

  let contract = ERC20.bind(tokenAddress)
  
  // Initialize decimalValue as i32 (since decimals are typically small integers)
  ...
  // Convert i32 to BigInt when returning
...
export function createLiquiditySnapshot(position: LiquidityPosition, event: ethereum.Event): void {
  ...
  snapshot.reserve0 = pair.s_reserveA
  snapshot.reserve1 = pair.s_reserveB
